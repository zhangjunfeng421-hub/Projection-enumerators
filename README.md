...
## GSOA_opt.R
This code file only has one useful function, "GSOA_opt(polyprimitive, s, K = (s - 1), q = (length(polyprimitive)-1), add = 0)" to construct the optimal GSOA(s^p,K*(s^p-1)/(s-1),s^p,t). 
